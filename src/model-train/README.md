...
- `GET /api/model-train/status/:trackingId` → `{ status: pending|running|completed|failed, progress: number, result: any, error: string, modelId: string }`
  - Returns status and progress of specified tracking id 
...
A celery worker `celery-app` will take on the queued job, runs the model training pipeline and continuously update the job status in Redis and MariaDB, where clients can poll the real-time status and progress. 
